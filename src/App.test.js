...
test('renders show data when select season is clicked', async () => {
    mockFetchShow.mockResolvedValueOnce(episodesData);
    const { getByText, debug, queryAllByTestId } = render(<App />)

    //wait for data to come in
    await waitFor(() => expect(queryAllByTestId(/poster-img/i)).toHaveLength(1));

    //pressing select a season
    const dropDown = getByText(/select a season/i);
    userEvent.click(dropDown);

    //selecting season 1
    const selectSeasonOne = getByText(/season 1/i);
    userEvent.click(selectSeasonOne);

    //expect there are 3 episodes
